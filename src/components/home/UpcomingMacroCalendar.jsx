    ...
    <div className={cn("h-full flex flex-col rounded", 
      ...
      <div className={cn("px-2 py-1 border-b", 
        ...
        <div className="flex items-center justify-between">
          <h3 className={cn("text-[10px] font-semibold uppercase tracking-wider", 
            ...
            isDark ? "text-neutral-500" : "text-gray-700")}>UPCOMING MACRO CALENDAR</h3>
          <Button
            size="sm"
            variant="ghost"
      ...
      {isLoading ? (
        ...
        <div className="flex-1 overflow-y-auto">
          <div className={cn("flex items-center text-[8px] font-semibold uppercase tracking-wider px-2 py-1", 
            isPastel ? "bg-[#4A4D6C] text-[#A5A8C0]" :
            isDark ? "bg-[#1C1C1C] text-neutral-600" : "bg-gray-200 text-gray-700")}>
            ...
            {events.map((event, idx) => (
              <div
                ...
                className={cn(
                  ...
                  idx < events.length - 1 && (isPastel ? "border-b border-[#4A4D6C]" : "border-b border-[#1F1F1F]")
                )}
              >
                <div className={cn("w-12 font-mono truncate", 
                  isPastel ? "text-[#7B7E9C]" :
                  isDark ? "text-neutral-700" : "text-gray-500")}>{event.date}</div>
                <div className={cn("w-10 font-mono", 
                  isPastel ? "text-[#9B9EBC]" :
                  isDark ? "text-neutral-600" : "text-gray-600")}>{event.time}</div>
                <div className={cn("w-8 font-mono font-semibold", 
                  isPastel ? "text-[#D0D2E0]" :
                  isDark ? "text-neutral-500" : "text-gray-700")}>{event.currency}</div>
                <div className="w-4 flex items-center">
                  <div className="w-2 h-2" style={{ backgroundColor: getImpactColor(event.impact) }}></div>
                </div>
                <div className={cn("flex-1 min-w-0 truncate font-medium", 
                  isPastel ? "text-[#D0D2E0]" :
                  isDark ? "text-neutral-500" : "text-gray-700")}>{event.event}</div>
                <div className={cn("w-16 text-right font-mono tabular-nums", 
                  isPastel ? "text-[#9B9EBC]" :
                  isDark ? "text-neutral-600" : "text-gray-600")}>{event.forecast || '-'}</div>
                <div className={cn("w-16 text-right font-mono tabular-nums", 
                  isPastel ? "text-[#9B9EBC]" :
                  isDark ? "text-neutral-600" : "text-gray-600")}>{event.previous || '-'}</div>
                <div className={cn("w-16 text-right font-mono tabular-nums font-semibold", 
                  isPastel ? "text-white" :
                  isDark ? "text-neutral-400" : "text-gray-800")}>{event.actual || '-'}</div>
              </div>
            ))}
          </div>
        </div>
