...
export default function StructuralChangePanel({ theme }) {
  ...
  useEffect(() => {
    const cached = localStorage.getItem('structural_change');
    if (cached) {
...
Select 5-8 articles that best signal structural industry changes. Return article indices.`;
    ...
    localStorage.setItem('structural_change', JSON.stringify(selected));
    setIsLoading(false);
  };
      ...
      {articles.length === 0 && !isLoading ? (
        ...
        <div className={cn("flex-1 p-2 space-y-1 overflow-y-auto custom-scrollbar", 
          ...
          {articles.map((article, idx) => (
            ...
            >
              <div className={cn("text-[9px] mb-1 inline-block px-1.5 py-0.5 rounded font-medium uppercase", 
              ...
              <div className={cn("text-[9px] mt-0.5 flex items-center gap-1.5", 
                ...
                {article.pubDate && (
                  <span>{format(new Date(article.pubDate), 'MMM d, h:mm a')}</span>
                )}
              </div>
            </a>
          ))}
